...
import ReviewerSurveyListItem from './ReviewerSurveyListItem';
...
export default function ReviewerHomePage() {
  ...
  useEffect(() => {
    ...
    listSurveys(metamask, Role.reviewer).then((values) => setSurveyList(values))
  }, [])
      ...
      <Container sx={{ paddingTop: '100px' }}>
        ...
        <Grid container spacing={4} sx={{ width: '100%' }}>
          {surveyList.map(survey => (
            <ReviewerSurveyListItem key={survey.title} survey={survey} />
          ))}
        </Grid>
      </Container>
